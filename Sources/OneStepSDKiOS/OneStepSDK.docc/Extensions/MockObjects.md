...
final class MyViewModelTests: XCTestCase {
    ...
    override func setUp() {
        ...
        mockSDK = MockOneStep(
            ...
            initialAuthState: .identified(OSTPatientId(rawValue: "test-user-001"))
        )
    ...
    func testMeasurementQuery() throws {
        guard case .success(let motionLab) = mockSDK.motionLab() else {
            return XCTFail("motionLab unavailable")
        }
    ...
    func testMonitoringBlockedState() {
        ...
        guard case .success(let monitoring) = mockSDK.monitoring() else { return }
        XCTAssertTrue(monitoring.runtimeState.isBlocked)
    }
}
```
...
// Change mock SDK auth state mid-test
let mockOneStep = MockOneStep()
mockOneStep.setMockAuthState(.identified(OSTPatientId(rawValue: "new-user")))
```
